...
Пользователю best-effort уходит уведомление в бота, что тариф изменён (если он запускал
бота; недоставка не влияет на `200`). При активации токена (`/start` или `/activate`)
все админы получают уведомление в бота: хто, який користувач і тариф.
